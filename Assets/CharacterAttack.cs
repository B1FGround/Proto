...
public class CharacterAttack : MonoBehaviour
{
    ...
    public void Initialized(TeamManager.CharacterType characterType)
    {
        ...
        switch (characterType)
        {
            ...
            case TeamManager.CharacterType.Magician:
                chainLightning = Instantiate(Resources.Load("ChainLightning")) as GameObject;
    ...
    public void Equip(TeamManager.CharacterType selected)
    {
        ...
        if (equippedItem.ContainsKey(IEquipable.EquipSocket.Armor))
        {
            ...
            var armorObj = Instantiate(Resources.Load("Equip/" + armorName), Vector3.zero, Quaternion.identity, armorSlot.transform) as GameObject;
        ...
        if (equippedItem.ContainsKey(IEquipable.EquipSocket.Bottom))
        {
            ...
            var armorObj = Instantiate(Resources.Load("Equip/" + armorName), Vector3.zero, Quaternion.identity, bottomSlot.transform) as GameObject;
            armorObj.transform.localPosition = Vector3.zero;
        }
        if (equippedItem.ContainsKey(IEquipable.EquipSocket.Leg))
        {
            ...
            var armorObj = Instantiate(Resources.Load("Equip/Left" + armorName), Vector3.zero, Quaternion.identity, leftLegSlot.transform) as GameObject;
            armorObj.transform.localPosition = Vector3.zero;

            armorObj = Instantiate(Resources.Load("Equip/Right" + armorName), Vector3.zero, Quaternion.identity, rightLegSlot.transform) as GameObject;
            armorObj.transform.localPosition = Vector3.zero;
        }
    }
}